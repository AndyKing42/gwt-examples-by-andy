...
//--------------------------------------------------------------------------------------------------
private void createBookClub(final String name, final String desc) {
  int id = _nextId++;
  BookClub bookClub = new BookClub(desc, id, name, 0);
  _bookClubMap.put(id, bookClub);
} // createBookClub()
//--------------------------------------------------------------------------------------------------
private static final String[] Genres            = new String[] {"Adventure", "Art and Design",
    "Autobiography and Memoir", "Biography", "Business and Finance", "Chick Lit", "Classics",
    "Comics and Graphic Novel", "Craft and Hobbies", "Crime Fiction", "Fantasy", "Fiction", "Film",
    "Health, Mind, and Body", "Historical Fiction", "History", "Horror", "House and Garden",
    "Philosophy", "Poetry", "Politics", "Religion", "Romance", "Science and Nature",
    "Science Fiction", "Short Stories", "Sport and Leisure"};
...
private void createTestData() {
  ...
  for (int bookClubCount = 0; bookClubCount < NumberOfBookClubs; ++bookClubCount) {
    ...
    genreLocationSet.add(genre + "~" + location);
    String bookClubName = genre + " " + type + " of " + location;
    createBookClub(bookClubName, bookClubName);
  }
} // createTestData()
//--------------------------------------------------------------------------------------------------
@Override
...
public List<BookClub> selectAllBookClubs() {
  ...
  Collections.sort(result, new Comparator<BookClub>() {
    ...
    public int compare(final BookClub bookClub1, final BookClub bookClub2) {
      return bookClub1.getName().compareToIgnoreCase(bookClub2.getName());
    } // compare()
  });
  return result;
} // getBookClubList()
