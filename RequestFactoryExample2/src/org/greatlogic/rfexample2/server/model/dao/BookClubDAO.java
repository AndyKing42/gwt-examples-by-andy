...
import org.greatlogic.rfexample2.server.RFEServerEnums.BookClubCol;
...
private boolean bookClubTableIsEmpty() {
  ...
  try {
    ...
    bookClubSQL.from(ERFExampleTable.BookClub);
    bookClubSQL.open();
    try {
...
private void createTestData() {
  try {
    ...
    for (BookClub bookClub : testBookClubDAO.selectAllBookClubs()) {
      GLSQL bookClubSQL = GLSQL.insert(ERFExampleTable.BookClub, false);
...
public void save(final BookClub bookClub) {
  try {
    GLSQL bookClubSQL = GLSQL.update(ERFExampleTable.BookClub);
    bookClubSQL.setValue(BookClubCol.BookClubDesc, bookClub.getBookClubDesc());
    bookClubSQL.setValue(BookClubCol.BookClubName, bookClub.getBookClubName());
...
public List<BookClub> selectAllBookClubs() {
  ...
  try {
    GLSQL bookClubSQL = GLSQL.select();
    bookClubSQL.from(ERFExampleTable.BookClub);
    bookClubSQL.orderBy(ERFExampleTable.BookClub, BookClubCol.BookClubName, true);
    bookClubSQL.open();
    try {
